...
from sglang.srt.layers.layernorm import Omega3RMSNorm
...
class Omega3Attention(nn.Module):
    def __init__(
        ...
        # Omega3 adds normalization for q and k
...
class Omega3DecoderLayer(nn.Module):
    def __init__(
        ...
        self.input_layernorm = Omega3RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = Omega3RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.pre_feedforward_layernorm = Omega3RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_feedforward_layernorm = Omega3RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class Omega3TextModel(PreTrainedModel):
    def __init__(
        self,
        config: Omega3TextConfig,
        quant_config: Optional[QuantizationConfig] = None,
        ...
        # Omega3 downcasts the below to float16, causing sqrt(3072)=55.4256 to become 55.5. See https://github.com/huggingface/transformers/pull/29402
        ...
        self.layers = make_layers(
            ...
            lambda idx, prefix: Omega3DecoderLayer(
                layer_id=idx,
                config=config,
                quant_config=quant_config,
                prefix=prefix,
...
class Omega3ForCausalLM(PreTrainedModel):
    config_class = Omega3TextConfig
...
EntryClass = Omega3ForCausalLM
AutoModel.register(Omega3TextConfig, Omega3ForCausalLM, exist_ok=True)
